...
function DayRow({
  ...
  return (
    locked ? <div className="flex min-h-16 items-center gap-3 rounded-md px-3 py-2 opacity-60">
      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-surface-muted text-label font-bold text-fg-3">{day.day + 1}</span>
    ...
    >
      <span
        className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-md text-label font-bold ${
      ...
      {isToday && <span className="rounded-md bg-white px-2 py-1 text-micro font-bold uppercase tracking-wide text-emerald">Today</span>}
...
export function CoursePath({ courseSlug, perDay, today }: CoursePathProps) {
  ...
  return (
    <section aria-labelledby="course-path-heading" className="flex h-full flex-col rounded-md border border-border bg-white p-4 shadow-card sm:p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="eyebrow text-emerald">Path</p>
          <h2 id="course-path-heading" className="mt-1 text-h3 text-fg">Day by day.</h2>
        </div>
      ...
      {perDay.length === 0 ? (
        ...
        <>
          {/* Mobile: collapsed by default so the dashboard stays short. */}
          <details className="group mt-4 lg:hidden">
            <summary className="flex min-h-12 cursor-pointer list-none items-center justify-between gap-3 rounded-md bg-surface-muted px-3 text-small font-bold text-fg-2 marker:content-none">
              View all {perDay.length} days
              <ChevronDown className="h-4 w-4 text-fg-3 transition-transform group-open:rotate-180" aria-hidden />
            </summary>
            <div className="mt-2 divide-y divide-border">{dayList}</div>
          </details>
          {/* Desktop: always visible, scrolls inside the fixed-height card. */}
          <div className="mt-4 hidden min-h-0 flex-1 divide-y divide-border overflow-y-auto lg:block">{dayList}</div>
        </>
      )}
